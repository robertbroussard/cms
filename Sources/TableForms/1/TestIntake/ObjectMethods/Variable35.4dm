  // Family button on Summary Screen - creates FamilyFile record if needed;
		...
		Case of 
			: ([IntakeFile]AddPartyFile'APType="ATC@")
				...
				If ([IntakeFile]AddPartyFile'APStAddress>"")
					$zipper:=[IntakeFile]AddPartyFile'APZip
					If (Length:C16($zipper)>5)
						$zipper:=Insert string:C231($zipper;"-";6)
					End if 
					vATCAddr:=[IntakeFile]AddPartyFile'APStAddress+", "+[IntakeFile]AddPartyFile'APCity+",  "+[IntakeFile]AddPartyFile'ApState+"  "+$zipper
				Else 
					vATCAddr:=""
				End if 
